...
Objetivo: Persistir estado como sequencia de eventos imutaveis e separar modelos de leitura e escrita para sistemas que exigem auditoria, rastreabilidade ou logica de dominio complexa.
...
- Trilha de auditoria completa e requisito (financeiro, saude, compliance)
- Precisa reconstruir estado em qualquer ponto no tempo ("como estava em 15/jan?")
- Dominio naturalmente baseado em eventos (pedidos, transacoes, reservas)
- Precisa derivar multiplas visoes dos mesmos dados

Event Sourcing NAO vale quando:
- CRUD simples sem regras de negocio complexas
- Time sem experiencia e prazo curto
- Dados que nao precisam de historico (cache, sessoes, logs)
