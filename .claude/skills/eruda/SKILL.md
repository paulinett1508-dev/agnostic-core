...
3. **Registers** a custom Eruda "Report" tab with a copy button
4. **Generates** a Markdown report with: URL, device, viewport, errors, warnings, logs
5. **Copies** to clipboard for pasting into Claude Code
...
Para projetos sem Vite, adicionar o script completo (com captura de logs + Report plugin) direto antes do `</body>` no HTML principal. O script é o mesmo conteúdo da IIFE acima, sem o wrapper do Vite plugin. Identificar o arquivo HTML principal conforme a stack:
